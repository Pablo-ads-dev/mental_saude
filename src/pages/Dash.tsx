...
export default function Dash() {
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen w-full bg-background selection:bg-primary/20">
            <SideBar />
            ...
            <main className="flex-1 transition-all duration-300 
                ...
                {/* 1. Header de Acolhimento */}
                <header className="p-6 md:p-8 rounded-[--radius] bg-card shadow-card border border-border/50">
                    ...
                    {/* Moods com scroll horizontal no mobile se necessário */}
                    <div className="flex justify-between items-center mt-8 bg-muted/50 p-4 md:p-6 rounded-2xl border border-border overflow-x-auto gap-4 no-scrollbar">
                        {moods.map((m) => (
                            <button
                                key={m.value}
                                onClick={() => setMood(m.value)}
                                className={`flex flex-col items-center gap-2 min-w-[60px] transition-all duration-500 ${mood === m.value
                            ...
                            >
                                ...
                                <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-center ${mood === m.value ? "text-primary" : "text-muted-foreground"}`}>
                                    {m.label}
                                </span>
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                        ...
                        <Card className="glass-card shadow-card hover:shadow-card-hover transition-all duration-500 rounded-[--radius]">
                            <CardContent className="p-6 md:p-8 space-y-6 md:space-y-8">
                                <div className="flex justify-between items-end">
                                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-tighter">Meta Diária</span>
                                    <span className="text-2xl font-display font-black text-primary">65%</span>
                                </div>
                                <Progress value={65} className="h-3 md:h-4 bg-muted" />
                    ...
                    <section className="space-y-4">
                        ...
                        <Card className="overflow-hidden border-none shadow-card-hover gradient-hero text-white min-h-[220px] md:h-[240px] rounded-[--radius] relative group">
                            <CardContent className="relative z-20 p-6 md:p-8 h-full flex flex-col justify-between">
                                <div>
                                    ...
                                    <h3 className="text-xl md:text-2xl font-display font-bold leading-tight">Equilíbrio e Ansiedade</h3>
                                </div>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mt-4">
                                    ...
                                    <Button className="w-full sm:w-auto bg-white text-purple-dark hover:bg-white/90 font-bold rounded-xl px-6 shadow-xl">
                                        Continuar
                                    </Button>
                                </div>
                            </CardContent>
                ...
                {/* 4. Insight do Dia */}
                <section>
                    <div className="p-6 md:p-10 rounded-[--radius] glass-card border-primary/20 shadow-card relative overflow-hidden group">
                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
                            <div className="bg-primary/10 p-4 md:p-5 rounded-3xl group-hover:rotate-12 transition-transform duration-500">
                                <Brain
                                    className="text-primary w-8 h-8 md:w-10 md:h-10"
                                    strokeWidth={2}
                                />                            </div>
                            <div className="text-center md:text-left">
                                <p className="text-xl md:text-3xl font-display font-bold text-foreground leading-tight italic">
                                    "Cuidar da mente é um ato de <span className="text-gradient-purple">coragem.</span>"
                                </p>
                                <p className="text-muted-foreground text-xs md:text-sm mt-3 font-semibold tracking-wide">— Pílula de Autocuidado MenteSã</p>
                            </div>
                        </div>
                    </div>
                </section>
...
function HabitIcon({ icon, label, done = false }: { icon: React.ReactNode, label: string, done?: boolean }) {
    return (
        <div className="flex flex-col items-center gap-3 group cursor-pointer">
            <div className={`p-5 rounded-3xl relative transition-all duration-500 ${done
